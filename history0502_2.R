...
library(dplyr)
# データセットから上から15行を削除する
c_tra_data2 <- slice(c_tra_data, -c(1:15))
# subsetではselectで列を切り取れるのに、dplyrでは
# スライスで行を切り取ることができる。罠

# 1から365ずつ大きくなる31列のデータセットを作成する
my_data2 <- data.frame(matrix(seq(1, 5857, by = 366), ncol = 1, nrow = 16))
# 年度が通し番号で判断されているため、年度を保存した上で大体同じ
tra_data2 <- cbind(my_data2,c_tra_data2)
# データセットの一部の列の名前を変更する
names(tra_data2)[1] <- c("new_date")

# selectはベクトルを入れると、列を当たり前に決してくる
# View(c_tra_data)

tra_data2$new_date <- as.Date(tra_data2$new_date, format = "%Y-%m-%d", origin = "2003-01-01")


# フッティングに移る
degree <- 4 # 4次関数にフィッティングする例
tra_data2$x <- as.numeric(tra_data2$new_date)
# 日付を数字にする（通し番号）
# そして代入する。
tra_data2$x2 <- tra_data2$x^2
# 二乗して入れる
tra_data2$x3 <- tra_data2$x^3
# 三乗して入れる
tra_data2$x4 <- tra_data2$x^4
# 四乗して入れる
...
fit <- lm(距離 ~ x + x2 + x3 + x4, data = tra_data2[, c("距離", "x", "x2", "x3", "x4")])
...
# プロットの作成
library(ggplot2)
ggplot(tra_data2, aes(x = new_date, y = 距離)) +
  geom_point() +
